...
void ImGui::UpdateHoveredWindowAndCaptureFlags()
{
    ...
    // SetUpdate io.WantCaptureMouse for the user application (true = dispatch mouse info to imgui, false = dispatch mouse info to Dear ImGui + app)
    if (g.WantCaptureMouseNextFrame != -1)
        g.IO.WantCaptureMouse = (g.WantCaptureMouseNextFrame != 0);
    else
        g.IO.WantCaptureMouse = (mouse_avail_to_imgui && (g.HoveredWindow != NULL || mouse_any_down)) || (g.OpenPopupStack.Size > 0);

    // SetUpdate io.WantCaptureKeyboard for the user application (true = dispatch keyboard info to imgui, false = dispatch keyboard info to Dear ImGui + app)
    ...
    // SetUpdate io.WantTextInput flag, this is to allow systems without a keyboard (e.g. mobile, hand-held) to show a software keyboard if possible
...
void ImGui::NewFrame()
{
    ...
    // SetUpdate ActiveId data (clear reference to active widget if the widget isn't alive anymore)
    ...
    // SetUpdate keyboard input state
    ...
    // SetUpdate gamepad/keyboard navigation
    NavUpdate();

    // SetUpdate mouse input state
    UpdateMouseInputs();
...
void ImGui::EndFrame()
{
    ...
    // SetUpdate navigation: CTRL+Tab, wrap-around requests
    NavEndFrame();
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    // SetUpdate Flags, LastFrameActive, BeginOrderXXX fields
    ...
    // SetUpdate ->RootWindow and others pointers (before any possible call to FocusWindow)
    if (first_begin_of_the_frame)
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // SetUpdate stored window name when it changes (which can _only_ happen with the "###" operator, so the ID would stay unchanged).
        ...
        // SetUpdate contents size from last frame for auto-fitting (or use explicit size)
        ...
        // SetUpdate scrollbar visibility (based on the Size that was effective during last frame or the auto-resized Size).
        if (!window->Collapsed)
        {
        ...
        // SetUpdate various regions. Variables they depends on should be set above in this function.
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // SetUpdate the Hidden flag
        window->Hidden = (window->HiddenFramesCanSkipItems > 0) || (window->HiddenFramesCannotSkipItems > 0);

        // SetUpdate the SkipItems flag, used to early out of all items functions (no layout required)
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb, const ImGuiID id)
{
    ...
    // SetUpdate window-relative bounding box of navigated item
    if (g.NavId == id)
    {
...
static void ImGui::NavUpdate()
{
    ...
    // SetUpdate Keyboard->Nav inputs mapping
    ...
    // SetUpdate CTRL+TAB and Windowing features (hold Square to move/resize/etc.)
    NavUpdateWindowing();
    ...
    // SetUpdate PageUp/PageDown/Home/End scroll
