...
void main()
{
    const vec3 diffuseColor_kd = texture(tex_Diffuse, vTexCoord).xyz;
    const vec3 specularColor_ks = texture(tex_Specular, vTexCoord).xyz;
    ...
    vec3 specular = coEfficients.b * specularColor_ks * pow(max(dot(R,V), 0), specularColor_ks.r * specularColor_ks.r
    * 255);
